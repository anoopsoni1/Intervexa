...
router.route("/make-premium").post(verifyJWT, makePremium)
router.route("/templates").post(verifyJWT, requireAdmin, upload.single("image"), createTemplate).get(getTemplates)
router.route("/templates/:id").get(getTemplateById).patch(verifyJWT, requireAdmin, updateTemplate).delete(verifyJWT, requireAdmin, deleteTemplate)
router.route("/make-admin").post(verifyJWT, requireAdmin, makeAdmin)
