...
/** Resume 2: left — Summary, Achievements, Skills (two columns); right — Education, Experience, Projects. */
  ...
  return (
    <article className={`${DOCUMENT_CLASS} max-w-4xl flex flex-col overflow-visible bg-[#fafaf9] print:bg-[#fafaf9] border border-zinc-200 print:border-zinc-300`}>
      <div className="w-full flex flex-row flex-wrap items-start justify-between gap-4 px-4 sm:px-6 pt-5 pb-4 border-b border-zinc-200">
        ...
        <div className="space-y-2.5 text-xs text-zinc-700 text-right">
          {data?.phone && (
            <a
          ...
          {data?.linkedin && (
            ...
            >
              <ExternalLink size={12} className="shrink-0 text-[#0f766e]" />{" "}
          ...
          {data?.github && (
            ...
            >
              <ExternalLink size={12} className="shrink-0 text-[#0f766e]" /> {String(data.github).replace(/^https?:\/\//i, "")}
            </a>
          )}
        </div>
      </div>
      ...
      <div className="w-full flex flex-col md:flex-row print:flex-row flex-1 min-h-0">
        <div className="w-full md:w-[40%] print:w-[40%] min-h-0 flex flex-col px-4 sm:px-5 py-5 border-b md:border-b-0 md:border-r border-zinc-200 bg-white/50 print:bg-white/80 font-medium" >
          {summary && (
          ...
          {achievementsList.length > 0 && (
            <section className="resume-section-avoid-break mb-5">
              <h2 className={`flex items-center gap-2 ${SECTION_HEAD}`}>
                <Award size={12} className="shrink-0 text-[#0f766e]" /> Achievements
              </h2>
              <ul className="space-y-0 list-none pl-0 font-medium">
                {achievementsList.map((a, i) => (
                  <li key={i} className="text-[11px] text-zinc-700 leading-snug flex items-start gap-1.5">
          ...
          {skillsList.length > 0 && (
            <section className="resume-section-avoid-break">
              ...
              <ul className="m-0 grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px] text-zinc-700 list-none p-0 font-medium items-start">
                {skillsList.map((s, i) => {
        ...
        <div className="w-full md:w-[60%] print:w-[60%] min-h-0 flex flex-col px-4 sm:px-5 py-4 bg-[#fafaf9] print:bg-[#fafaf9]">
          {(data?.education?.trim() || educationParsed) && (
            <section className="mb-4 resume-section-avoid-break">
              <h2 className={SECTION_HEAD}>Education</h2>
              <div className="space-y-1">
                {educationParsed ? (
                  <div className="space-y-0.5">
                    {educationParsed.degree && <p className="text-[11px] text-zinc-800 font-medium">{educationParsed.degree}</p>}
                    {educationParsed.institution && <p className="text-xs text-zinc-600">{educationParsed.institution}</p>}
                    {educationParsed.dates && <p className="text-[10px] text-zinc-500">{educationParsed.dates}</p>}
                  </div>
                ) : (
                  <p className="text-xs text-zinc-600 whitespace-pre-wrap">{data.education?.trim()}</p>
                )}
              </div>
            </section>
          )}
          ...
          {projectsList.length > 0 && (
            <section className="mb-4">
              ...
              <ul className="space-y-2 list-none pl-0 font-medium">
                      ...
                      <div className="min-w-0">
                        ...
                        {p.link ? (
                          <span className="block mt-0.5">
                            <ResumeProjectLink url={p.link} className="text-[#0f766e] underline print:text-black font-normal" />
                          </span>
                        ) : null}
                        {p.description ? (
          ...
          {languageList.length > 0 && (
            <section className="mb-4">
              <h2 className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-[#0f766e] pb-1 mb-2 border-b border-zinc-300">
                Languages
              </h2>
              <ul className="space-y-1 list-none pl-0 text-[11px] text-zinc-700">
