...
/** Resume 3: left — Summary, Achievements, Skills; right — Education, Experience, Projects, Language. */
export default function Resume3Layout({ data }) {
  ...
  const languageLinesParsed = parseLanguageProficiencyList(data?.languageProficiency);
  const defaultLanguageLines = languageLinesParsed.length > 0 ? languageLinesParsed : ["English"];
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row print:flex-row flex-1 min-h-0 px-4 sm:px-6 pb-5">
        ...
        <div className="w-full md:w-[38%] print:w-[38%] min-h-0 flex flex-col pr-4 border-b md:border-b-0 md:border-r border-zinc-200 pb-4 md:pb-0">
          ...
          {achievementsList.length > 0 && (
            <section className="mb-4">
              ...
              <ul className="space-y-0 list-none pl-0 text-xs text-zinc-700">
          ...
          <section className="mb-4">
            ...
            <ul className="m-0 grid grid-cols-2 gap-x-3 gap-y-1 list-none p-0 text-xs text-zinc-700 items-start">
              {defaultSkills.map((s, i) => (
                <li key={i} className="flex gap-2 leading-snug min-w-0">
                  <span className="w-1.5 h-1.5 rounded-full bg-black shrink-0 mt-1.5" aria-hidden />
                  <span className="min-w-0 wrap-break-word">{typeof s === "string" ? s : s?.label ?? ""}</span>
                </li>
              ))}
            </ul>
          </section>
        ...
        {/* Right column: Education, Experience, Projects, Language */}
        <div className="w-full md:w-[62%] print:w-[62%] min-h-0 flex flex-col pl-0 md:pl-4 pt-4 md:pt-0">
          <section className="mb-4">
          ...
          <section className="mb-4">
            ...
            <ul className="space-y-1 list-none pl-0 text-xs text-zinc-700">
          ...
          <section>
            ...
            <ul className="space-y-0.5 list-none pl-0 text-xs text-zinc-700">
              {defaultLanguageLines.map((lang, i) => (
                <li key={i} className="flex gap-2 leading-snug">
                  <span className="w-1.5 h-1.5 rounded-full bg-black shrink-0 mt-1.5" aria-hidden />
                  {lang}
                </li>
