 ...
 * Used by Portfolio, ResumeView, and PortfolioDesignView.
 */

import { RESUME_ACHIEVEMENTS_MAX } from "./resumeAchievements.js";
...
const SECTION_HEADERS = {
  ...
  achievements: [
    /(?:^|\n)\s*(?:ACHIEVEMENTS?|Achievements?|AWARDS?|Awards?|ACCOMPLISHMENTS?|Accomplishments?)\s*:?\s*/im,
  ],
  education: [
...
export function parseResume(text = "") {
  ...
  let experienceRaw = extractSection(raw, SECTION_HEADERS.experience, [
    /\n\s*(?:ACHIEVEMENTS?|AWARDS?|REFERENCES?|PROJECTS?|EDUCATION|SKILLS?|CERTIFICATIONS?)\s*:?\s*/im,
  ]);
  let projectsRaw = extractSection(raw, SECTION_HEADERS.projects, [
    /\n\s*(?:EDUCATION|SKILLS?|EXPERIENCE|CERTIFICATIONS?)\s*:?\s*/im,
  ]);
  let achievementsRaw = extractSection(raw, SECTION_HEADERS.achievements, [
    /\n\s*(?:REFERENCES|PROJECTS?|EDUCATION|LANGUAGE\s*PROFICIENCY|LANGUAGES?|SKILLS?)\s*:?\s*/im,
  ]);
  ...
  const achievements = achievementsRaw
    ? achievementsRaw
        .split("\n")
        .map((l) => stripBullet(stripAsterisks(l)).trim())
        .filter(Boolean)
        .filter((s) => !/^(?:ACHIEVEMENTS?|AWARDS?|ACCOMPLISHMENTS?)$/i.test(s))
