...
export default function ModernResumeDesignViewPage() {
    ...
    const fetchResumeTemplates = async () => {
      try {
        ...
        if (data?.success && Array.isArray(data?.data)) {
          ...
          const styleIsModern = (t) => {
            const s = (t?.style || "").toLowerCase();
            return s === "modern" || s === "";
          };
          const resumes = data.data
            .filter((t) => t.type !== "portfolio" && styleIsModern(t))
            .sort((a, b) => {
        ...
        <main className="mx-auto w-full max-w-8xl px-4 sm:px-6 lg:px-8 py-10 sm:py-14 md:py-20 min-h-[60vh]">
          <header className="mb-10 sm:mb-14 flex flex-col items-center text-center">
            <div className="mb-4 flex items-center gap-2 rounded-full bg-indigo-500/15 border border-indigo-400/20 px-4 py-2 text-indigo-300">
              ...
              <span className="text-sm font-medium">Modern resume designs</span>
            </div>
            ...
            <p className="mt-4 max-w-lg text-base sm:text-lg text-indigo-200/70 leading-relaxed">
              Pick a modern layout for your resume. Each design uses your saved details—add them first if you haven’t.
            </p>
          </header>

          {user ? (
            <div className="mb-8 rounded-2xl border border-indigo-400/30 bg-indigo-500/10 backdrop-blur-sm px-5 py-4 text-indigo-200 text-sm flex items-start gap-3">
              <div className="shrink-0 w-9 h-9 rounded-full bg-indigo-500/20 flex items-center justify-center mt-0.5">
              ...
              <div className="ml-auto shrink-0">
                <Link
                  to="/add-details"
                  className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-xs font-semibold text-white hover:bg-indigo-500 transition-all"
                >
                  Edit details
                </Link>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8">
            ...
            {!resumeLoading && resumeTemplates.length === 0 && !resumeError && (
              <div className="col-span-full flex flex-col items-center justify-center py-16 px-4 rounded-2xl border border-white/10 bg-white/5 text-center">
                ...
                <p className="text-zinc-400 text-sm sm:text-base">No modern resume templates yet.</p>
                <p className="text-zinc-500 text-sm mt-1">Upload templates with style “Modern” in the admin panel, or try another category.</p>
              </div>
            )}
            {!resumeLoading &&
          ...
          <nav className="mt-12 sm:mt-16 flex flex-wrap justify-center items-center gap-2 sm:gap-4">
            ...
            >
              <ArrowLeft className="h-4 w-4" /> Modern (resume or portfolio)
            </Link>
