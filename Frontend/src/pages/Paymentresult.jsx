...
function PaymentResult() {
  ...
  useEffect(() => {
    ...
    const razorpay_order_id = sessionStorage.getItem("razorpay_order_id");
    const razorpay_payment_id = sessionStorage.getItem("razorpay_payment_id");
    const razorpay_signature = sessionStorage.getItem("razorpay_signature");
    ...
    const payment = async () => {
      try {
        ...
        if (data.payment_status?.toLowerCase() === "success") {
          ...
          sessionStorage.removeItem("razorpay_signature");
              ...
              {paymentData && (
                <div className="space-y-4 mb-6">
                  ...
                  <div className="rounded-xl bg-black/20 border border-white/5 divide-y divide-white/5 overflow-hidden">
                    ...
                    <DetailRow icon={Hash} label="Payment ID" value={renderValue(paymentData.payment_id)} />
                    <DetailRow icon={Banknote} label="Amount" value={`₹${renderValue(paymentData.amount)}`} highlight />
                    <DetailRow icon={CreditCard} label="Currency" value={renderValue(paymentData.currency)} />
                    <DetailRow icon={CreditCard} label="Gateway" value="Razorpay" />
                    <DetailRow icon={CreditCard} label="Method" value={renderValue(paymentData.method)} />
                    <DetailRow
                      icon={Calendar}
                      label="Payment time"
                      value={
                        paymentData.created_at
                          ? new Date(paymentData.created_at * 1000).toLocaleString()
                          : "—"
                      }
                    />
                    <DetailRow icon={MessageSquare} label="Status" value={renderValue(paymentData.status)} />
                  </div>
