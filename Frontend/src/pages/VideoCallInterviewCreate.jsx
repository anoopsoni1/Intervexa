...
function VideoCallInterviewCreate() {
  ...
  const { status: usageStatus } = useUsageStatus(!authChecking);
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    if (liveBlocked) {
      setError(`Daily live interview limit reached. ${formatResetsLabel(usageStatus?.liveInterview?.resetsAt)}`);
      return;
    }
  ...
  return (
    <div ref={createPageRef} className="flex flex-col min-h-screen">
        <AppHeader />
          ...
          <div className="max-w-xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="create-form rounded-2xl border border-indigo-500/30 bg-black/60 backdrop-blur-xl p-6 sm:p-7 space-y-4 shadow-xl shadow-indigo-500/20">
              ...
              <div className="create-actions flex flex-wrap gap-3 pt-4 mt-2 border-t border-white/10">
                <Link
                  to="/dashboard/interviews"
                  className="flex-1 min-w-0 inline-flex items-center justify-center gap-2 py-3 rounded-xl bg-white/5 text-slate-200 font-semibold text-center hover:bg-white/10 transition hover:scale-[1.02] active:scale-[0.98]"
                >
                ...
                <button
                  type="submit"
                  disabled={submitting || liveBlocked}
                  className="flex-1 min-w-0 inline-flex items-center justify-center gap-2 py-3 rounded-xl bg-indigo-600 text-white font-semibold hover:bg-indigo-500 transition disabled:opacity-60 hover:scale-[1.02] active:scale-[0.98] disabled:hover:scale-100"
                >
                  {submitting ? (
                    <>
