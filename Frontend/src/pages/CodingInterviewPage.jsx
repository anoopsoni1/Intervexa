...
export default function CodingInterviewPage() {
  ...
  }, [handleEnterFullscreen, codingBlocked]);
  ...
  useEffect(() => {
    ...
    async function pollJobResult(jobId) {
      ...
      while (!cancelled && Date.now() - start < JOB_POLL_TIMEOUT_MS) {
        try {
          const pollRes = await fetch(`${API_BASE_URL}/api/v1/job/${jobId}`);
          const pollJson = await pollRes.json();
          if (cancelled) return;
          const stored = pollJson?.data ?? pollJson;
          if (stored?.status === "completed") {
            const list = stored?.result?.data?.questions ?? [];
            setQuestions(Array.isArray(list) ? list : []);
            setCurrentIndex(0);
            return;
          }
          if (stored?.status === "failed") {
            setQuestionError(stored?.error || "Failed to generate questions");
            return;
          }
        } catch (e) {
      ...
      if (!cancelled) setQuestionError("Questions are taking longer than expected. Please try again.");
    }
    async function fetchQuestions() {
      ...
      try {
        ...
        if (cancelled) return;
        if (!res.ok) {
          setQuestionError(json?.message || json?.error || "Failed to load questions");
          return;
        }
        const data = json?.data ?? json;
        // Backend may return 202 with jobId when using queue (Redis); then we must poll for result
      ...
      try {
        ...
        if (res.status === 429) {
          ...
          let msg =
            json?.message ||
            json?.error ||
            "Daily coding interview limit reached. Try again after the next UTC day reset.";
          ...
          <div className="max-w-lg w-full rounded-2xl border border-white/10 bg-slate-900/80 backdrop-blur-sm p-8 shadow-xl">
            ...
            {codingBlocked ? (
              <div className="mb-6 rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
                <p className="font-semibold text-amber-200">Daily limit reached</p>
                <p className="mt-1 text-amber-100/90">
                  {formatResetsLabel(usageStatus?.codingInterview?.resetsAt)}
                </p>
                <Link
                  to="/coding-interview/start"
                  className="mt-3 inline-flex text-xs font-semibold text-amber-300 hover:text-white underline underline-offset-2"
                >
            ...
            <button
              ...
              disabled={codingBlocked}
              title={
                codingBlocked
                  ? formatResetsLabel(usageStatus?.codingInterview?.resetsAt)
                  : undefined
              }
              className="w-full rounded-xl bg-emerald-600 py-3.5 font-semibold text-white shadow-lg shadow-emerald-500/20 hover:bg-emerald-500 transition-all flex items-center justify-center gap-2 disabled:opacity-45 disabled:cursor-not-allowed disabled:hover:bg-emerald-600"
            >
