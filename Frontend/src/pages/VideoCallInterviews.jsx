...
function VideoCallInterviews() {
  ...
  const { status: usageStatus } = useUsageStatus(!authChecking && !!user);
  ...
  if (authChecking) {
    return (
      <div className="flex flex-col min-h-screen">
        <AppHeader />
        <main className="flex-1 px-4 py-8 max-w-8xl mx-auto w-full space-y-4">
        ...
        <main className="flex-1 py-6 sm:py-8 px-3 sm:px-4">
          <div className="max-w-8xl mx-auto w-full">
            <div ref={headerRef} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              ...
              {liveBlocked ? (
                <span
                  className="inline-flex flex-col items-center justify-center gap-1 rounded-xl border border-amber-500/40 bg-amber-500/10 px-4 py-2.5 text-amber-200 text-sm font-medium shrink-0 w-full sm:w-auto cursor-not-allowed"
                  title={formatResetsLabel(usageStatus?.liveInterview?.resetsAt)}
                >
                  <span className="inline-flex items-center gap-2">
                    <FiPlus className="w-5 h-5 opacity-60" />
                    New interview (limit reached)
                  </span>
                  <span className="text-xs font-normal text-amber-300/90">
                    {formatResetsLabel(usageStatus?.liveInterview?.resetsAt)}
                  </span>
                </span>
              ) : (
                <Link
