...
import { parsedToDetailPayload } from "../utils/detailApi.js";
import AppHeader from "../components/layout/AppHeader";
import AppFooter from "../components/layout/AppFooter";
import { API_BASE, UPLOAD_PROMO_VIDEO_URL } from "../config";
...
const ACCEPTED_TYPES = ["PDF", "DOC", "DOCX", "PNG", "JPG", "WEBP"];
...
const features = [
  ...
  },
  {
    ...
    desc: "Practice your interview skills with AI-powered mock interviews.",
    icon: LayoutTemplate,
...
function UploadPage() {
  ...
  const saveDetailMutation = useMutation({
    mutationFn: async ({ accessToken, payload }) => {
      ...
      if (!res.ok) {
        throw new Error("Failed to save extracted resume details");
      }
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (!file) {
      setMessage("Please select a PDF, Word file, or a resume image (PNG, JPG, WebP)");
      setMessageType("error");
      return;
    }
    ...
    if (forceOcr && !isImageResume(file)) {
    ...
    <div className="min-h-screen">
      ...
      <div className="min-h-[90vh] flex flex-col items-center justify-center px-4 sm:px-6 py-8 sm:py-12">
        <div className="w-full max-w-6xl mx-auto text-center mb-8 sm:mb-10">
          ...
          >
            We extract text from your file (including OCR for scans and photos) and help you improve it for recruiters and ATS systems.
          </motion.p>
        </div>
        ...
        >
          <form onSubmit={handleUpload} className="space-y-5">
            ...
            {user && file && !isImageResume(file) && (
              <label className="flex items-center justify-center gap-2 cursor-pointer text-sm text-zinc-400 hover:text-zinc-300 select-none">
                <input
                  type="checkbox"
                  checked={forceOcr}
                  onChange={(e) => setForceOcr(e.target.checked)}
                  className="rounded border-white/30 bg-white/10 text-amber-500 focus:ring-amber-500/40"
                />
                <span>Scanned PDF — run full OCR (slower, use if text extraction is wrong)</span>
              </label>
            )}
              ...
              >
                {loading ? (
                  <span className="flex gap-2 justify-center items-center">
                    ...
                    <span>
                      {isImageResume(file) || forceOcr
                        ? "Uploading & OCR scanning…"
        ...
        >
          <div className="max-w-8xl mx-auto">
            ...
            >
              <motion.div
                variants={fadeUp}
                className="w-full lg:max-w-xl xl:max-w-2xl shrink-0"
              >
